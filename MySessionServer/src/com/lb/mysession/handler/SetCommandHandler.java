...
public class SetCommandHandler extends AbstractCommandHandler {
	...
	public void doCommand(SessionContext context){
    	...
    	SessionMap map = context.getSessionMap();
    	String sessionId=message.getSessionId();
    	SessionBean session=map.getSession(sessionId);
		
    	if(session==null){
			session=new SessionBean(sessionId);
			map.putSession(session);
		}
		
		session.setAttribute(attributeKey , message.getValue() );
		writeResult(context, COMMAND_SETRESULT,SUCCESS,"");
	}
}
